...
import { getUserByStudentId, getTodaysGamesWithPredictions, submitMultiplePredictions } from '@/lib/api';
...
type SelectedPick = { gameId: number; predictedTeamId: number; teamName: string; };
...
export default function HomePage() {
    ...
    const [selectedPicks, setSelectedPicks] = useState<Map<number, SelectedPick>>(new Map());
    ...
    // --- UI HANDLERS ---
    const handleSelectPick = (gameId: number, predictedTeamId: number, teamName: string) => {
    ...
    return (
        <div className="w-full max-w-4xl mx-auto p-8">
            <h1 className="text-4xl font-bold text-center text-gray-800 mb-10">Today's Predictions</h1>
            ...
            {user && isLoading && <p className="text-center text-gray-800">Loading games...</p>}
                ...
                <div className="space-y-6">
                    ...
                    {selectedPicks.size > 0 && (
                        <div className="mt-8 text-center">
                            <button onClick={() => setShowConfirmation(true)} className="px-8 py-4 bg-blue-600 text-white font-bold rounded-lg shadow-lg">
            ...
            {showConfirmation && (
                <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center">
                    <div className="bg-white p-8 rounded-lg shadow-2xl">
                        <h3 className="text-xl font-bold mb-4 text-gray-900">Confirm Your Predictions</h3>
                        <ul className="list-disc list-inside mb-6 text-gray-700">
                            {Array.from(selectedPicks.values()).map(pick => (
                                <li key={pick.gameId}>You picked the <span className="font-bold">{pick.teamName}</span> to win.</li>
                            ))}
                        </ul>
                        <div className="flex justify-center gap-4">
                            <button onClick={() => setShowConfirmation(false)} className="px-6 py-2 bg-gray-300 text-gray-800 rounded-lg">Cancel</button>
                            <button onClick={handleConfirmSubmission} className="px-6 py-2 bg-blue-600 text-white rounded-lg">Confirm & Submit</button>
                        </div>
                    </div>
                </div>
            )}
